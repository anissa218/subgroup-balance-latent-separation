...
class HAMLitModule(LightningModule):
    """Example of a `LightningModule` for MIMIC classification.
    ...
    def on_load_checkpoint(self, checkpoint: dict) -> None:
        """Reset ViT classification head after restoring checkpoint."""
        if self.trainer is not None and self.trainer.training:
            if hasattr(self.net, "model") and hasattr(self.net.model.heads, "head"):
                num_features = self.net.model.heads.head.in_features
                output_size = self.net.model.heads.head.out_features
                self.net.model.heads.head = torch.nn.Linear(num_features, output_size)
                torch.nn.init.kaiming_normal_(self.net.model.heads.head.weight, nonlinearity="relu")
                torch.nn.init.constant_(self.net.model.heads.head.bias, 0)
                print("ViT classification head reset after checkpoint load.")



    # if it doesn't work can debug with this:
    ...
    def training_step(
        ...
        """
        x, y, _ = batch
        loss, preds, targets = self.model_step((x,y)) # don't need to pass dicom_id
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        bal_acc = self.val_bal_acc.compute()  # get current val balanced acc
        self.val_bal_acc_best(bal_acc)  # update best so far val balanced acc
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        :return: A dict with the model predictions and with the ground truth target labels.
        """
        x, y, dicom_id = batch
    ...
    def on_test_epoch_end(self) -> None:
        """Do nothing special. Required only if you do not return from `test_epoch_end`."""
        pass
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        # for sequential schedulers (to do warmup + cosine)
        if isinstance(scheduler_cfg, partial) and scheduler_cfg.func == torch.optim.lr_scheduler.SequentialLR:
            ...
            # instantiate each inner scheduler with optimizer
...
if __name__ == "__main__":
    _ = HAMLitModule(None, None, None, None)
