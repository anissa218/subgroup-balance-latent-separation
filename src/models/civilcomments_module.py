...
class CIVILCOMMENTSLitModule(LightningModule):
    ...
    def on_load_checkpoint(self, checkpoint: dict) -> None:
        """Hook called after loading a checkpoint. Resets classifier head so it starts from scratch."""
        if self.trainer is not None and self.trainer.training:
            if hasattr(self.net, "classifier"):
                hidden_size = self.net.hidden_size
                output_size = self.net.classifier.out_features
                self.net.classifier = torch.nn.Linear(hidden_size, output_size)
                torch.nn.init.kaiming_normal_(self.net.classifier.weight, nonlinearity="relu")
                torch.nn.init.constant_(self.net.classifier.bias, 0)
                print("BERT classifier head reset after checkpoint load.")
...
if __name__ == "__main__":
    _ = CIVILCOMMENTSLitModule(None, None, None, None)
