...
class MNISTCOLOURLitModule(LightningModule):
    ...
    def __init__(
        ...
        """Initialize a `MNISTCOLOURLitModule`.
    ...
    def training_step(
        ...
        """
        x, y, _ = batch
        loss, preds, targets = self.model_step((x,y)) # don't need to pass idx
    ...
    def predict_step(
        ...
        _,logits = self.forward(x)
        preds = torch.argmax(logits, dim=1)
        return {"ids": idx, "preds": preds, "targets": y, "logits": logits}


if __name__ == "__main__":
    _ = MNISTCOLOURLitModule(None, None, None, None)
