...
def get_metrics_one_run(preds_metadata_df, attr, experiment, n_group_0, n_group_1, run_id, loss = torch.nn.CrossEntropyLoss()):
...
def get_summary_df(experiment_string, test_metadata_df, required_lr, required_data_name, attributes, fixed_n = 500, variable_n = [0,20,200,500,1500,5000,10000], max_count = 9):
    all_run_records = []
    for attr in attributes:
        for n in variable_n:
            ...
            experiment = experiment_string+'_'+attr+'_0_'+str(n_group_0)+'_1_'+str(n_group_1)
            ...
            for run_folder in os.listdir(results_dir):
                ...
                if all(x in config['data']['train_data_path'] for x in [attr,required_data_name]) and config['model']['optimizer']['lr'] == required_lr: # filter out runs not for correct target, modified proportion, and not using the full dataset
                    ...
                    run_record = get_metrics_one_run(preds_metadata_df,attr, experiment,n_group_0, n_group_1,count)
                    all_run_records.append(run_record)

                    count += 1

            # repeat with n_group_1 = fixed
            n_group_0 = n
            n_group_1 = fixed_n
    ...
    summary_df["subgroup"] = summary_df["experiment"].str.extract(r"^expand_(.+?)_0_")
    summary_df['n_group_0'] = summary_df['experiment'].str.split('_').str[-3].astype(int)
    summary_df['n_group_1'] = summary_df['experiment'].str.split('_').str[-1].astype(int)
...
def plot_expand_multicol(df, subgroups=("A","S","Random_binary"), fixed=500, log_x=True):
    fig, (axL, axR) = plt.subplots(1, 2, figsize=(12, 6), sharey=False)
    colors = plt.cm.tab10.colors   # 10 distinct
    palette = {sg: colors[i] for i, sg in enumerate(subgroups)}
    ...
    for sg in subgroups:
        ...
        left = sub[sub["n_group_0"] == fixed]
        if not left.empty:
            x = left["n_group_1"].to_numpy()
            y = left[("balanced_acc_group_0", "mean")].to_numpy()
            e = left[("balanced_acc_group_0", "std")].to_numpy()
            o = np.argsort(x)
            axL.errorbar(x[o], y[o], yerr=e[o], marker="o", capsize=3,
                         label=sg, color=palette.get(sg),alpha=0.7)

        right = sub[sub["n_group_1"] == fixed]
        if not right.empty:
            x = right["n_group_0"].to_numpy()
            y = right[("balanced_acc_group_1", "mean")].to_numpy()
            e = right[("balanced_acc_group_1", "std")].to_numpy()
            o = np.argsort(x)
            axR.errorbar(x[o], y[o], yerr=e[o], marker="o", capsize=3,
                         label=sg, color=palette.get(sg),alpha=0.7)

    if log_x:
        axL.set_xscale("symlog", linthresh=1) 
        axR.set_xscale("symlog", linthresh=1) # to plot when x = 0, might not be the best way
        # axL.set_xscale("log") 
        # axR.set_xscale("log") 

    axL.set_xlabel(f"n_group_1 (n_group_0={fixed})")
    axR.set_xlabel(f"n_group_0 (n_group_1={fixed})")

    axL.set_ylabel("acc_group_0")
    axR.set_ylabel("acc_group_1")
